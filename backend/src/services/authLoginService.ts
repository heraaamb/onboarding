...
export async function checkUserDetails(data: any) {
    ...
    const result = await pool.query(`
        SELECT u.user_id, u.password_hash, u.role, u.email, u.department_id, e.emp_id
        FROM users u
    ...
    const { password_hash, role, user_id, email, department_id, emp_id } = result.rows[0];
    ...
    const payload = { id: user_id, email, role, department_id, emp_id };
    // // Debugging
